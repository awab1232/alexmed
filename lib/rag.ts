...
const STOPWORDS = new Set([
  "في", "من", "على", "إلى", "الى", "عن", "مع", "هو", "هي", "هذا", "هذه",
  "ذلك", "تلك", "ما", "ماذا", "لماذا", "ليش", "شو", "كيف", "متى", "أين",
  "وين", "هل", "أو", "او", "ثم", "كل", "بعض", "أي", "اي", "لا", "لم", "لن",
  "قد", "كان", "كانت", "يكون", "التي", "الذي", "الذين", "اللي", "عند",
  "بين", "بعد", "قبل", "حتى", "لو", "إذا", "اذا", "انا", "أنا", "انت",
  "أنت", "نحن", "هم", "لي", "لك", "اشرح", "اشرحلي", "وضح", "ممكن", "يعني",
  "the", "a", "an", "of", "to", "in", "on", "for", "and", "or", "is", "are",
  "was", "were", "be", "what", "why", "how", "when", "where", "which",
  "who", "does", "do", "did", "can", "could", "with", "about", "this",
  "that", "these", "those", "it", "its", "me", "explain", "please",
]);
...
export async function getScopeOverview(session: {
  ...
  if (session.scope === "subject" && session.subjectId) {
    const rows = await db.execute<{ name: string; fileName: string | null }>(sql`
...
export function buildRagSystemPrompt(scope: ChatScope): string {
  return [
    `You are "مساعد مِرآة", a brilliant, warm study tutor inside an Arabic study app. The student is studying ${scopeLabel(scope)} of their own uploaded material; its overview and the most relevant pages (SOURCE EXCERPTS, each tagged with its page number) are given in their message.`,
