...
const blockSchema = {
  ...
  properties: {
    kind: { type: "string", enum: ["definition", "bullet_group", "alert", "comparison", "algorithm", "image"] },
    heading: { type: "string" },
    bodyEn: { type: "string" },
    bodyAr: { type: "string" },
    items: { type: "array", items: { type: "string" } },
    tone: { type: "string", enum: ["default", "high_yield", "warning", "clinical"] },
    sourcePages: { type: "array", items: { type: "integer" } },
  },
  required: ["kind", "heading", "bodyEn", "bodyAr", "items", "tone", "sourcePages"],
};
        ...
        pages: {
          ...
          items: {
            ...
            properties: {
              ...
              layout: { type: "string", enum: ["overview", "sections", "comparison", "algorithm", "exam"] },
              sourcePages: { type: "array", items: { type: "integer" } },
...
export function buildMedicalNoteComposerMessages(input: {
  ...
  visuals: { pageNumber: number; assetType: string; descriptionEn: string | null; descriptionAr: string | null }[];
}): Message[] {
  const pageText = input.pages.map(page => `[PAGE ${page.page}]\n${page.text}`).join("\n\n");
  const visuals = input.visuals.map(v => `[PAGE ${v.pageNumber}] ${v.assetType}: ${v.descriptionEn ?? ""} / ${v.descriptionAr ?? ""}`).join("\n");
  return [
    {
      role: "system",
      content: [
        ...
        "Create 3-8 coherent pages. Each page should have 2-5 blocks, short bullets, visual hierarchy, and real sourcePages. Put thresholds, durations, classifications, treatment steps, and danger signs into high-yield or warning blocks.",
...
export function parseMedicalNotePages(content: unknown, validPages: number[]): MedicalNotePage[] {
  const parsed = parseJsonResponse(content) as unknown as { pages: MedicalNotePage[] };
  const valid = new Set(validPages);
  return parsed.pages.map(page => ({
    ...page,
    sourcePages: page.sourcePages.filter(p => valid.has(p)),
    blocks: page.blocks.map(block => ({ ...block, sourcePages: block.sourcePages.filter(p => valid.has(p)) })),
  }));
}
